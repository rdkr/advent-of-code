...
class IntComp:
    ...
    def __init__(self, initial_input):
        self.inputs = initial_input
        self.output = 0
        self.data = DATA.copy()
        self.data.extend([0]*1000)
        self.pointer = 0
    ...
    def _update_state(self):
        ...
        params_modes_string = str(self.data[self.pointer])[:-2].zfill(IntComp.params[self.op])
        param_modes = [int(x) for x in params_modes_string][::-1]
        params = self.data[self.pointer + 1 : self.pointer + 1 + len(param_modes)]
        self.params_tuples = list(zip(param_modes, params))


    def _get_value(self, pos):
        if self.params_tuples[pos][0] == 1:
            return int(self.params_tuples[pos][1])
        if self.params_tuples[pos][0] == 2:
            return int(self.data[self.params_tuples[pos][1] + self.relative_base])
        return int(self.data[self.params_tuples[pos][1]])


    def _set_value(self, param, value):
        if self.params_tuples[param][0] == 2:
            self.data[self.params_tuples[param][1] + self.relative_base] = value
        else:
            self.data[self.params_tuples[param][1]] = value
    ...
    def run(self):
        while True:
            if self.op == 99:
                ...
                return
            if self.op == 1:
                self._set_value(2, self._get_value(0) + self._get_value(1))
            elif self.op == 2:
                self._set_value(2, self._get_value(0) * self._get_value(1))
            elif self.op == 3:
                if self.inputs:
                    self._set_value(0, self.inputs.pop(0))
                else:
                    return
            elif self.op == 4:
                self.output = self._get_value(0)
                print(self.output)
            elif self.op == 5 and self._get_value(0) != 0:
                self.pointer = self._get_value(1)
                self._update_state()
                continue
            elif self.op == 6 and self._get_value(0) == 0:
                self.pointer = self._get_value(1)
                self._update_state()
                continue
            elif self.op == 7:
                self._set_value(2, (1 if self._get_value(0) < self._get_value(1) else 0))
            elif self.op == 8:
                self._set_value(2, (1 if self._get_value(0) == self._get_value(1) else 0))
            elif self.op == 9:
                self.relative_base = self.relative_base + self._get_value(0)

            self.pointer = self.pointer + IntComp.params[self.op] + 1
            self._update_state()
